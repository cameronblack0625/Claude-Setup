...
C_BLACK='\033[38;5;236m'  # dark gray/black for nothink
C_YELLOW='\033[38;5;220m'  # yellow for megathink
# Rainbow colors for ultrathink
C_R1='\033[38;5;196m'  # red
C_R2='\033[38;5;208m'  # orange
C_R3='\033[38;5;226m'  # yellow
C_R4='\033[38;5;46m'   # green
C_R5='\033[38;5;51m'   # cyan
C_R6='\033[38;5;21m'   # blue
C_R7='\033[38;5;93m'   # purple
C_R8='\033[38;5;201m'  # magenta
C_R9='\033[38;5;196m'  # red
C_R10='\033[38;5;208m' # orange
case "$COLOR" in
...
# Get thinking mode from settings.local.json
thinking_tokens=""
settings_local="$HOME/.claude/settings.local.json"
if [[ -f "$settings_local" ]]; then
    thinking_tokens=$(jq -r '.env.MAX_THINKING_TOKENS // empty' "$settings_local" 2>/dev/null)
fi

# Map thinking tokens to display string with colors
thinking_display=""
case "$thinking_tokens" in
    0)
        thinking_display="${C_BLACK}nothink${C_RESET}"
        ;;
    4000)
        thinking_display="${C_ACCENT}think${C_RESET}"
        ;;
    10000)
        thinking_display="${C_YELLOW}megathink${C_RESET}"
        ;;
    31999)
        # Rainbow striped ultrathink
        thinking_display="${C_R1}u${C_R2}l${C_R3}t${C_R4}r${C_R5}a${C_R6}t${C_R7}h${C_R8}i${C_R9}n${C_R10}k${C_RESET}"
        ;;
esac
...
# Build output: Model | Thinking | Dir | Branch (uncommitted) | Context
output="${C_ACCENT}${model}${C_RESET}"
[[ -n "$thinking_display" ]] && output+="${C_GRAY} | ${thinking_display}"
output+="${C_GRAY} | 📁${dir}"
...
if [[ -n "$transcript_path" && -f "$transcript_path" ]]; then
    ...
    plain_output="${model}"
    [[ -n "$thinking_display" ]] && plain_output+=" | ultrathink"  # use longest thinking mode for calculation
    plain_output+=" | 📁${dir}"
